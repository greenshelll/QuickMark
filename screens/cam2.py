...
class CameraWidget(FloatLayout):
    def __init__(self, **kwargs):
        ...
        self.draw = lambda image: cv2.drawContours(image, [], -1, (0, 255, 0), 3)
        self.draw_previous = lambda image: cv2.drawContours(image, [], -1, (0, 255, 0), 3)
    ...
    def process_frame(self, frame_data):
        ...
        def infer(frame_data):


            #print("INFERING")
            
            if self.is_currently_processing == False:
                ...
                try:
                    ...
                    if self.cs.boxes.rectangles is not None:
                        ...
                        if not self.good_angles(npbox,85): # display threshold
                            ...
                            if not self.good_angles(npbox, 45):
                                ...
                                self.draw = lambda image: cv2.rectangle(image, (x, y), (x + w, y + h), (255,0,175), 3)
                            #__________________________________________________
                        else:
                            db.p('drawing contours')
                            self.draw = lambda image: cv2.drawContours(image, self.cs.boxes.rectangles, -1, (0, 255, 0), 4)
                    ...
                    self.is_currently_processing = False
        #print(self.is_currently_processing)
        
        if  True:
            
            #threading.Thread(target=lambda:infer(frame_data))
            
            thread = threading.Thread(target=lambda: infer(frame_data))
        
            thread.start()
        #print('last',self.is_currently_processing)
        ...
        try:
            ...
            if image is not None:
                if not self.is_currently_processing:
                    self.draw_previous(image)
                else:
                    self.draw(image)
                    self.draw_previous == self.draw
            db.p("RENDER DONE")
        except Exception as e:
            print(e)
            image = frame_data
    ...
    def extract_frame(self, instance):
        ...
        if texture:
           ...
           """ if self.set_is_done == False:
                with self.frame_image.canvas.before:
                # Set the rotation angle (in degrees) and rotation center (use image center)
                    Rotate(angle=90, origin=self.frame_image.center)
                self.set_is_done = True
            self.frame_image.texture = self.camera.texture"""
           pass
