...
def get_bubbles(BubbleGetter_obj, BoxGetter_obj, CaptureSheet_obj, boxes_num):
    ...
    try:
        ...
        for i, contour in enumerate(contours):
            ...
            if len(approx) == 4 and cv2.contourArea(contour) > benchmark_area-(benchmark_area*0.2)  and cv2.contourArea(contour) < benchmark_area*200:
                ...
                if h != 0:  # Avoid division by zero
                    ratio = w / h
                    if (ratio >= 0.7 and ratio <= 1.3) or (cv2.contourArea(contour) > benchmark_area*30):
                        not_inside = True
